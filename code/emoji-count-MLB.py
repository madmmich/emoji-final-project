...
pos_emoji = pandas.read_csv('skimmed-tweets-MLB.csv', encoding = 'utf-8')


pos_emoji = open("skimmed-tweets-MLB.csv", "r")
emoji_tweets = csv.DictReader(pos_emoji)

tweet_content = []

for col in emoji_tweets:
    tweet_content.append(col['Tweets MLB']) 
...
full_file = open("emoji_count_MLB.csv", "w")
...
with open('emoji_count_MLB.csv', 'r') as read_obj, \
        open('updated_emoji_MLB.csv', 'w', newline='') as write_obj:
