...
pos_emoji = pandas.read_csv('skimmed-tweets-airbnb.csv', encoding = 'utf-8')


pos_emoji = open("skimmed-tweets-airbnb.csv", "r")
emoji_tweets = csv.DictReader(pos_emoji)

tweet_content = []

for col in emoji_tweets:
    tweet_content.append(col['Tweets Airbnb']) 
...
full_file = open("emoji_count_airbnb.csv", "w")
...
with open('emoji_count_airbnb.csv', 'r') as read_obj, \
        open('updated_emoji_airbnb.csv', 'w', newline='') as write_obj:
