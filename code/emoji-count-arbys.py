...
pos_emoji = pandas.read_csv('skimmed-tweets-arbys.csv', encoding = 'utf-8')


pos_emoji = open("skimmed-tweets-arbys.csv", "r")
emoji_tweets = csv.DictReader(pos_emoji)

tweet_content = []

for col in emoji_tweets:
    tweet_content.append(col['Tweets Arbys']) 
...
full_file = open("emoji_count_arbys.csv", "w")
...
with open('emoji_count_arbys.csv', 'r') as read_obj, \
        open('updated_emoji_arbys.csv', 'w', newline='') as write_obj:
