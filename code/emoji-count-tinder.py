...
pos_emoji = pandas.read_csv('skimmed-tweets-tinder.csv', encoding = 'utf-8')


pos_emoji = open("skimmed-tweets-tinder.csv", "r")
emoji_tweets = csv.DictReader(pos_emoji)

tweet_content = []

for col in emoji_tweets:
    tweet_content.append(col['Tweets']) 
...
full_file = open("emoji_count.csv", "w")
...
with open('emoji_count.csv', 'r') as read_obj, \
        open('updated_emoji.csv', 'w', newline='') as write_obj:
