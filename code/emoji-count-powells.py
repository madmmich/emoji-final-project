...
pos_emoji = pandas.read_csv('skimmed-tweets-Powells.csv', encoding = 'utf-8')


pos_emoji = open("skimmed-tweets-Powells.csv", "r")
emoji_tweets = csv.DictReader(pos_emoji)

tweet_content = []

for col in emoji_tweets:
    tweet_content.append(col['Tweets Powells']) 
...
full_file = open("emoji_count_powells.csv", "w")
...
with open('emoji_count_powells.csv', 'r') as read_obj, \
        open('updated_emoji_powells.csv', 'w', newline='') as write_obj:
